...
export async function POST(req: Request) {
  ...
  const warning = usedCredit
    ? undefined
    : "AI generation timed out, so a fallback template was applied. No credit was used. Try again with a shorter prompt.";
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      send({ type: "progress", message: usedCredit ? "Building template structure..." : "Applying fallback template..." });
