...
export default function BillingSettingsPage() {
  ...
  async function startCheckout(target: "pro" | "team") {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(j.error ?? "Failed to open billing page");
        return;
      }
      if (j.url) window.location.href = j.url;
      else toast.error("Missing billing link.");
    } finally {
  ...
  return (
    <div className="mx-auto max-w-lg p-4 md:p-8">
      ...
      <h1 className="mt-4 font-heading text-2xl font-semibold tracking-[-0.02em]">Billing·credits</h1>
        ...
        <CardHeader>
          ...
          <CardDescription>
            {isFree
              ...
              : `You are currently on the ${plan === "team" ? "Team" : "Pro"} plan.`}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 text-sm text-muted-foreground">
          <p className="text-foreground">
            Credits left: <span className="font-semibold">{credits}</span>
          </p>
          <Button type="button" variant="secondary" size="sm" className="rounded-xl" disabled>
            {isFree ? "Current plan: Free" : `Current plan: ${plan === "team" ? "Team" : "Pro"}`}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card className="mt-6 rounded-xl border-border shadow-sm">
        <CardHeader>
          ...
          <CardDescription>Buy credits when you want to generate more templates with AI.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 sm:grid-cols-2">
          <div className="flex flex-col rounded-xl border border-border bg-card p-4 shadow-sm">
            ...
            >
              {checkoutBusy === "pro" ? "Connecting..." : "Buy"}
            </Button>
          </div>
          <div className="flex flex-col rounded-xl border border-border bg-card p-4 shadow-sm">
            ...
            >
              {checkoutBusy === "team" ? "Connecting..." : "Buy"}
            </Button>
          </div>
        </CardContent>
      </Card>
